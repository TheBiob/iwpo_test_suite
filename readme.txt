...
An iwpo test file is structured as follows:
----------------------------------------------
[configuration]
folder=
game=
...
is_gms       - boolean. Whether or not the GMS part of the tool should be copied and modified or only the gm8 part.
...
The server_packages section contains a list of pre-made packets the server can send. The game executable can request the server to send one of those packages by calling the @ServerPackage(id) function. The server will then respond with the corresponding package which the game can then process.
